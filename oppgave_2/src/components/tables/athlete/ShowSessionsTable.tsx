...
const ShowSessionsTable: React.FC = ( ) => {
  ...
  const editSession = (athleteId: string) => {
    router.push(`/edit/session/${athleteId}`);
  }
            ...
            {sortedSessions.map((session) => (
              <tr key={session.id}>
                ...
                <td className="border px-4 py-2">
                  ...
                  <button onClick={() => editSession(session.id)} className="ml-2 rounded bg-black px-4 py-2 text-white">
                    Endre
                  </button>
                </td>
              </tr>
...
export default ShowSessionsTable
